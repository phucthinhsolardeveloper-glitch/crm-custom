...
/**
 * Gộp Nhãn + Mạng xã hội vào 1 block không có divider giữa (theo wireframe).
 * Nhãn chỉ hiện khi có ≥ 1 label gắn vào KH.
 */
export function SidebarLabelsAndSocial({ customer }: { customer: CustomerRecord }) {
        ...
        <div className="flex items-center gap-3">
          {socialLinks.map(({ key, url, Icon, label }) =>
            url ? (
              ...
              <span key={key} title={`${label} - chưa có`} aria-label={`${label} chưa thiết lập`}>
                <Icon active={false} />
              </span>
            )
          )}
